...
### ⚠️ 메일 제목은 반드시 영문(ASCII)으로

사용 중인 `denomailer` 라이브러리는 제목에 한글처럼 UTF-8 인코딩 시 74바이트를 넘는 비ASCII 문자가 있으면,
RFC 2047 인코딩 과정에서 헤더 안에 들어가면 안 되는 줄바꿈이 섞여 들어가는 버그가 있습니다.
그러면 일부 메일 클라이언트(사내 Exchange/Outlook 등)가 메일 구조를 아예 못 읽고 원본 MIME 텍스트를 그대로 보여줍니다.
그래서 `index.ts`의 `subject`는 항상 영문으로 두고 있습니다 (본문 `html`은 한글이어도 문제없음).
제목을 한글로 바꾸고 싶다면 이 버그부터 고치거나(직접 RFC 2047 인코딩 로직 구현) 다른 라이브러리로 교체해야 합니다.
